...
const metricDescriptions: { [metric: string]: string } = {
  ...
  predictive_parity:
    "Measures consistency in positive predictive value across groups.",
  equalized_opportunity:
    "Measures gaps in true positive rate (TPR) between groups. Lower means more equal opportunity.",
  predictive_equality:
    "Measures consistency in false positive rate (FPR) across groups.",
  conditional_use_accuracy_equality:
    "Measures consistency in accuracy across groups.",
  accuracy_difference:
    "Difference in accuracy between privileged and unprivileged groups.",
  precision_difference:
    "Difference in precision between privileged and unprivileged groups.",
  recall_difference:
    "Difference in recall between privileged and unprivileged groups.",
  f1_difference:
    "Difference in F1 score between privileged and unprivileged groups.",
  ...
  representation_disparity:
    "Difference in representation in model outputs between groups.",
  prompt_fairness: "Fairness in prompt handling across groups.",
  balance_positive_class:
    "Balance in positive class predictions across groups.",
  balance_negative_class:
    "Balance in negative class predictions across groups.",
...
export default function BiasAndFairnessResultsPage() {
  ...
  >({});
  const [explorerDraftSelection, setExplorerDraftSelection] = useState<
    Record<string, boolean>
  >({});
  const [showFullJSON, setShowFullJSON] = useState(false);
  const [showMetricTooltip, setShowMetricTooltip] = useState<string | null>(
    null
  );
  ...
  const fairness_metrics: Record<string, MetricEntry> =
    metrics?.results?.fairness_metrics || {};
  const data_quality: DataQuality = useMemo(
    () => metrics?.results?.data_quality || {},
    [metrics?.results?.data_quality]
  );
  const metricsCfg: MetricsConfiguration =
    metrics?.results?.metadata?.metrics_configuration || {};
  ...
  const getMetricsToDisplay = useCallback(() => {
    ...
    const compassRecommended =
      metricsCfg.fairness_compass_recommended_metrics || [];

    // Combine user selected and compass recommended metrics, removing duplicates
    const displayMetrics = [
      ...new Set([...userSelected, ...compassRecommended]),
    ];

    return displayMetrics;
  }, [
    metricsCfg.user_selected_metrics,
    metricsCfg.fairness_compass_recommended_metrics,
  ]);

  // Check if a metric value is valid (not flagged as faulty)
  const isMetricValid = useCallback(
    (metricName: string, attribute: string) => {
  ...
  const getFairnessMetricsByAttribute = useCallback(
    (attribute: string): Record<string, number> => {
      ...
      if (metrics?.results?.fairness_metrics) {
        Object.entries(metrics.results.fairness_metrics).forEach(
          ([key, value]) => {
            if (
              key.endsWith(`_${attribute}`) &&
              value &&
              typeof value === "object" &&
              "value" in value &&
              ...
              if (
                ...
                Math.abs(metricValue) < EXTREME_VALUE_THRESHOLD
              ) {
  ...
  const sexMetricsAll = useMemo(
    () => getFairnessMetricsByAttribute("sex"),
    [getFairnessMetricsByAttribute]
  );
  const raceMetricsAll = useMemo(
    () => getFairnessMetricsByAttribute("race"),
    [getFairnessMetricsByAttribute]
  );

  // Apply explorer selection: if any metrics are selected, filter charts to those; else show all
  const selectedMetricNames = Object.keys(appliedSelection).filter(
    (k) => appliedSelection[k]
  );
  ...
  const sexMetrics = useMemo(
    () => filterBySelection(sexMetricsAll),
    [sexMetricsAll, filterBySelection]
  );
  const raceMetrics = useMemo(
    () => filterBySelection(raceMetricsAll),
    [raceMetricsAll, filterBySelection]
  );
  ...
  const hasDraftChanges = useMemo(() => {
    const keys = new Set<string>([
      ...Object.keys(appliedSelection),
      ...Object.keys(explorerDraftSelection),
    ]);
    for (const k of keys) {
  ...
  const handleMetricInfoClick = useCallback(
    (metricName: string) => {
      setShowMetricTooltip(
        showMetricTooltip === metricName ? null : metricName
      );
    },
    [showMetricTooltip]
  );
    ...
    async (isRetry = false) => {
      try {
        ...
        if (isDemo) {
          ...
          const data = await biasAndFairnessService.getBiasFairnessEvaluation(
            id as string
          );
          setMetrics(data);
        }
  ...
  useEffect(() => {
    if (metrics?.results?.metadata?.metrics_configuration) {
      const userSelected =
        metrics.results.metadata.metrics_configuration.user_selected_metrics ||
        [];
      const compassRecommended =
        metrics.results.metadata.metrics_configuration
        ...
        >
          <Box>
            ...
            <Typography sx={STYLES.secondaryText}>
              Comprehensive analysis of model fairness across protected
              attributes
            </Typography>
          </Box>
        </Box>
      ...
      <TabContext value={tab}>
              ...
              <Box>
                ...
                <Alert severity="info" sx={{ mb: 3 }}>
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    <strong>Plot interpretation:</strong> Values closer to 0
                    indicate better fairness. Green bars show good fairness
                    (&lt;{FAIRNESS_THRESHOLD_MODERATE}), yellow shows moderate
                    bias ({FAIRNESS_THRESHOLD_MODERATE}-
                    {FAIRNESS_THRESHOLD_SIGNIFICANT}), and red shows significant
                    bias (&gt;{FAIRNESS_THRESHOLD_SIGNIFICANT}).
                  </Typography>
                </Alert>
                      ...
                      {Plot ? (
                        <Plot
                          data={[
                            {
                              type: "bar",
                              x: Object.keys(sexMetrics).map((key) =>
                                key.replace(/_/g, " ")
                              ),
                              ...
                              marker: {
                                color: Object.values(sexMetrics).map((v) => {
                                  ...
                                  return Math.abs(numValue) >
                                    FAIRNESS_THRESHOLD_SIGNIFICANT
                                    ? COLORS.ERROR
                                    : Math.abs(numValue) >
                                      FAIRNESS_THRESHOLD_MODERATE
                                    ? COLORS.WARNING
                                    : COLORS.SUCCESS;
                                }),
                                line: { color: "#ffffff", width: 1 },
                              },
                              text: Object.values(sexMetrics).map((v) =>
                        ...
                        <BarChart
                          xAxis={[
                            {
                              scaleType: "band",
                              data: Object.keys(sexMetrics).map((key) =>
                                key.replace(/_/g, " ")
                              ),
                              tickLabelStyle: {
                                angle: 45,
                      ...
                      {Plot ? (
                        <Plot
                          data={[
                            {
                              type: "bar",
                              x: Object.keys(raceMetrics).map((key) =>
                                key.replace(/_/g, " ")
                              ),
                              ...
                              text: Object.values(raceMetrics).map((v) =>
                                typeof v === "number" ? v.toFixed(4) : "N/A"
                              ),
                              textposition: "outside",
                            },
                        ...
                        <BarChart
                          xAxis={[
                            {
                              scaleType: "band",
                              data: Object.keys(raceMetrics).map((key) =>
                                key.replace(/_/g, " ")
                              ),
                              tickLabelStyle: {
                                angle: 45,
                          ...
                          >
                            ...
                            >
                              {(() => {
                                ...
                                return (
                                  formattedMetric.charAt(0).toUpperCase() +
                            ...
                            >
                              {metricDescriptions[
                                metric as keyof typeof metricDescriptions
              ...
              <Box>
                ...
                >
                  ...
                  <Typography sx={{ ...STYLES.secondaryText, mt: 1 }}>
                    {data_quality.data_quality_score
                      ? data_quality.data_quality_score >= 0.8
                        ? "Excellent"
                        : data_quality.data_quality_score >= 0.6
                        ? "Good"
                        : data_quality.data_quality_score >= 0.4
                        ? "Fair"
                        : "Poor"
                      : "Unknown"}{" "}
                    quality assessment
                  </Typography>
                  {data_quality.flagged_metrics &&
                    Object.keys(data_quality.flagged_metrics).length > 0 && (
                      <Typography
                        sx={{
                      ...
                      >
                        {Object.keys(data_quality.flagged_metrics).length}{" "}
                        flagged metrics were not calculated
                      </Typography>
                    )}
                </Box>
              </Box>
        ...
        <TabPanel value="explorer" sx={{ ...tabPanelStyle, pt: 2 }}>
          <Box>
            ...
            <Typography
              variant="body2"
              sx={{ mb: 6, color: COLORS.TEXT_SECONDARY }}
            >
              Currently displaying {getMetricsToDisplay().length} metrics
              (user-selected + compass-recommended metrics).
            </Typography>
            <Grid container spacing={4}>
              <Grid item xs={12} md={4}>
                <Typography variant="body1" sx={{ mb: 2, ...STYLES.bodyText }}>
                  User-selected
                </Typography>
                <Stack spacing={3}>
                  {(metricsCfg.user_selected_metrics || []).map((m) => (
                    ...
                    >
                      ...
                      <Tooltip
                        title={
                          metricDescriptions[
                            m as keyof typeof metricDescriptions
                          ] || "No description available."
                        }
                        placement="top"
                      ...
                      >
                        <MetricInfoIcon
                          onClick={() => handleMetricInfoClick(m)}
                          size="small"
                        />
                      </Tooltip>
                    </Box>
              ...
              <Grid item xs={12} md={4}>
                ...
                <Stack spacing={3}>
                  {(metricsCfg.fairness_compass_recommended_metrics || []).map(
                    (m) => (
                      <Box
                        key={m}
                        sx={{
            ...
            <Box
              ...
              justifyContent="space-between"
              alignItems="center"
              sx={{ mt: 6, pt: 3, borderTop: "1px solid #e5e7eb" }}
            >
              <Typography variant="body2" sx={{ color: "#6b7280" }}>
                Select metrics to include/exclude them from charts on the Plots
                & Graphs tab.
              </Typography>
              <Button
                variant="contained"
        ...
        <TabPanel value="settings" sx={{ ...tabPanelStyle, pt: 2 }}>
          <Box>
              ...
              <Grid container spacing={1}>
                <Grid item xs={12} md={6}>
                  <Stack spacing={1}>
                    ...
                    >
                      <Typography
                        sx={{ fontSize: "12px", color: "#8594AC", pb: "2px" }}
                      >
                        Dataset
                      </Typography>
                      <Typography
                      ...
                      >
                        {metrics?.dataset_name ||
                          metrics?.results?.metadata?.dataset ||
                          "N/A"}
                      </Typography>
                    </Box>
                    <Box
                    ...
                    >
                      ...
                      >
                        {metrics?.model_name ||
                          metrics?.results?.metadata?.model ||
                          "N/A"}
                      </Typography>
                    </Box>
                    <Box
                    ...
                    >
                      ...
                      >
                        {(
                          metrics?.model_task ||
                          metrics?.results?.metadata?.model_task ||
                          "N/A"
                        )
                          .toString()
                          .replace("_", " ")}
                    ...
                    >
                      ...
                      >
                        {metrics?.created_at
                          ...
                          : metrics?.results?.metadata?.evaluation_timestamp
                          ? new Date(
                              metrics.results.metadata.evaluation_timestamp
            ...
            <Typography variant="body2" sx={{ color: COLORS.TEXT_SECONDARY }}>
              Configure default thresholds, sampling, and integration settings.
              (Coming soon)
            </Typography>
          </Box>
        </TabPanel>
