...
export async function getAssessmentTrackerMarkdown(
  ...
  const assessmentMD = `
${data.organizationName || "Konpurai"} assessment tracker report
========================

This report is generated by the Konpurai Assessment Tracker. It aims to provide a way to demonstrate their claims about the risks of their AI systems.
                  ...
                  const questionList =
                    subtopic.questions?.length > 0
                      ? subtopic.questions
                        .map(
                          (q, i) =>
                            `    ${i + 1}. __${q.question}__<br>${q.answer || "(No answer for this question.)"
                            }`
                        )
                        .join("\n")
