...
export async function getClausesAndAnnexesMarkdown(
  ...
  const markdown = `
${data.organizationName || "Konpurai"} clauses and annexes report
========================

This report is generated by the Konpurai ISO 42001 framework. It aims to provide a way to demonstrate their claims about the risks of their AI systems.
            ...
            annexes.annexCategories?.length > 0
              ? annexes.annexCategories
                .map((annexCategory, i) => {
                  ...
                  const res = `__A${annexes.annex_no}${i + 1}. ${annexCategory.title
                    }__ <br> Applicability: ${annexCategory.is_applicable
                    }<br> ${status}`;
                  return `  - ${res}\n`;
                })
                .join("\n")
